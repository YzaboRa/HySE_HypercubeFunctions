...
# 	"""
# 	An interactive class to select corresponding points in two images
...
# def CoRegisterHypercubeAndMask_Manual(RawHypercube, **kwargs):
# 	"""
# 	Apply landmark-based co-registration to a hypercube with interactive controls.
	
# 	Inputs:
# 		- RawHypercube
# 		- Wavelengths_list
# 		- kwargs:
# 			- StaticIndex = 0: Which frame to use a reference static image
# 			- Cropping = 0: Number of pixels to crop on all sides of all images
# 			- Blurring = True: If True, blurrs the image (gaussian blurring)
# 			- Sigma = 2: By how much the imnage is blurred
# 			- Order = False: If True, orders the hypercube according to Wavelengths_list
# 			- Wavelengths_list: List of wavelengths used (not sorted)
# 			- EdgeMask: If indicated, masks edges of the endoscopic image
# 			- AllReflectionsMasks: If indicated, one mask per frame (same size) to mask specular reflections
# 			- HideReflections: If True, masks reflections
# 			- DeviationThreshold = 150 : Threshold in pixels. If the distance between a point in the fixed image and the 
# 				corresponding point in the moving is larger than this threshold, a warning message will appear
# 			- AllLandmarkPoints: If indicated, the code will use those points for the registration instead of prompting
# 				the user to anotate the data manually


# 	Outputs:
# 		- Hypercube_sorted
# 		- AllTransforms_sorted: Transform applied (simpleITK)
# 		- CombinedMask: Combined mask (edges, reflections with transform applied)
# 		- AllLandmarkPoints_output: dict containing all the points anotated by the user. 
# 			Save this as np.savez(f'{PointsSavingPath}', AllLandmarkPoints, allow_pickle=True) to be able
# 			to reproduce the registration without having to manually anotate all points


# 	"""
# 	# --- Parameter Setup ---
# 	if kwargs.get('Help', False): print(inspect.getdoc(CoRegisterHypercubeAndMask_Manual)); return (None,)*4
...
# 			print(f'If asking to order the hypercube, you must provide a Wavelengths_list')
# 			return 0,0,0,0
# 	# SaveHypercube = kwargs.get('SaveHypercube', False)
# 	# PlotDiff = kwargs.get('PlotDiff', False)
# 	# SavingPath = kwargs.get('SavingPath')
...
# 		if EdgeMask is not None: EdgeMask = EdgeMask[Cropping:-Cropping, Cropping:-Cropping]
...
# 	if AllLandmarkPoints_input is not None:
# 		PromptUser = False
# 		fixed_landmarks = AllLandmarkPoints_input['fixed_points']
# 		all_moving_landmarks_input = AllLandmarkPoints_input['moving_points']
# 	else:
# 		PromptUser = True
# 		fixed_landmarks = []
...
# 		min_points = {'rigid': 2, 'similarity': 2, 'affine': 3}.get(kwargs.get('TransformType', 'affine').lower(), 3)
...
# 		transform = _compute_landmark_transform(fixed_landmarks, moving_landmarks_for_frame, kwargs.get('TransformType', 'Affine').lower())
		
# 		if transform is None:
# 			Hypercube.append(im_shifted); AllTransforms.append(None); continue
			
# 		im_static_se = _sitk.GetImageFromArray(im_static.astype(np.float64))
# 		im_shifted_se = _sitk.GetImageFromArray(im_shifted.astype(np.float64))
# 		im_coregistered_se = _sitk.Resample(im_shifted_se, im_static_se, transform, _sitk.sitkLinear, 0.0, im_shifted_se.GetPixelID())
# 		im_coregistered_final = _sitk.GetArrayFromImage(im_coregistered_se)
...
# 			ReflectionsMask_Shifted = AllReflectionsMasks[c, :, :]
# 			if ReflectionsMask_Shifted is not None:
# 				mask_se = _sitk.GetImageFromArray(ReflectionsMask_Shifted.astype(np.uint8))
# 				registered_mask_se = _sitk.Resample(mask_se, im_static_se, transform, _sitk.sitkNearestNeighbor, 0, mask_se.GetPixelID())
# 				im_coregistered_final[_sitk.GetArrayFromImage(registered_mask_se).astype(bool)] = np.nan
...
# 	if PostCropping is not None:
# 		Hypercube_sorted = Hypercube_sorted[:,PostCropping:-1*PostCropping, PostCropping*-1*PostCropping]
# 		CombinedMask = CombinedMask[PostCropping:-1*PostCropping, PostCropping*-1*PostCropping]
