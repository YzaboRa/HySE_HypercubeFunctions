...
def GetChannelInterplay(file, dark_means, buffer=10, HighPixVal=200, LowPixVal=50, return_mask=False, offset=0):
    """
    :param file: Channel Interplay Video File
    :param dark_means: Spatial Mean RGB Dark Reference
    :param buffer: How many Frames to ignore at either side of each peak
    :param HighPixVal: Cut-off value for ignoring saturated pixels
    :param LowPixVal: Cut-on value for ignoring dim pixels
    :param return_mask: Do you want to output the pixel intensity mask as well as the correction matrix? 
    :param offset: In case the video started well before or after the start of the first red sweep
    :return: 
    """""
    ...
    n_B, n_G, n_R, n_mix = len(BSweep), len(GSweep), len(RSweep), len(
        BGRTest[0])  # Number of exposure times for each sweep set
    ...
    if offset < 0:
        missing_red = np.round(offset / t_single)
    ...
    for i, col in enumerate(['b', 'g', 'r']):
        for rpt in [0, 1]:
            x_start = ([n_R + n_G, n_R, 0][i] * t_single) + t_repeat * rpt + offset
            if x_start >= 0:
                ...
                starts[i, rpt] = np.nan
    print(f'Offset:{offset}, Sweep Starts:{starts}')

    # Find start of mixed sweep
    x_start = (n_R + n_G + n_B) * t_single + offset
    ...
    img = HySE.Import.ImportData(file, RGB=True, Trace=False, TrimSat=False,
                                 Coords=[int(starts[i, rpt] + buffer), int(starts[i, rpt] + t_single - buffer)]).mean(
        axis=0)
    ...
    all_values, all_exps = [], []
    for rpt in [0, 1]:
        for i, col1 in enumerate(['b', 'g', 'r']):  # Get number of exposure times and start of trace
            values, startset, n = [], starts[i], [n_B, n_G, n_R][i]

            if np.isnan(startset[rpt]):  # Ignore missing sweeps
                print(f"Sweep {col1}{rpt} Missing")

            else:
                # Save Exposure Times for the Sweep
                exps_i = [BSweep, GSweep, RSweep][i]
